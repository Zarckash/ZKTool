Add-Type -AssemblyName System.Windows.Forms
[System.Windows.Forms.Application]::EnableVisualStyles()

$ErrorActionPreference = 'SilentlyContinue'
$ConfirmPreference = 'None'

# Run Script As Administrator
If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {
	Start-Process powershell.exe "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
	Exit
}

# Dark Or Light Theme
if ((Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize' -name AppsUseLightTheme | select -exp AppsUseLightTheme) -eq 0) {
    $formtextcolor = [System.Drawing.ColorTranslator]::FromHtml("#FFFFFF")
    $bgcolor = [System.Drawing.ColorTranslator]::FromHtml("#363636")
    $textcolor = [System.Drawing.ColorTranslator]::FromHtml("#99FFF0")
    $buttoncolor = [System.Drawing.ColorTranslator]::FromHtml("#464646")
} else {
    $formtextcolor = [System.Drawing.ColorTranslator]::FromHtml("#000000")
    $bgcolor = [System.Drawing.ColorTranslator]::FromHtml("#F3F3F3")
    $textcolor = [System.Drawing.ColorTranslator]::FromHtml("#A85EE9")
    $buttoncolor = [System.Drawing.ColorTranslator]::FromHtml("#FFFFFF")
}

$Form                            = New-Object system.Windows.Forms.Form
$Form.ClientSize                 = New-Object System.Drawing.Point(1050,700)
$Form.text                       = "Hidden"
$Form.StartPosition              = "Manual"
$Form.TopMost                    = $false
$Form.BackColor                  = $bgcolor
$Form.AutoScaleDimensions        = '192, 192'
$Form.AutoScaleMode              = "Dpi"
$Form.AutoSize                   = $True
$Form.ClientSize                 = '1050, 700'
$Form.FormBorderStyle            = 'FixedSingle'
$Form.Location                   = New-Object System.Drawing.Point(839, 199)

$iconBase64                      = 
''
$iconBytes                       = [Convert]::FromBase64String($iconBase64)
$stream                          = New-Object IO.MemoryStream($iconBytes, 0, $iconBytes.Length)
$stream.Write($iconBytes, 0, $iconBytes.Length)
$Form.Icon                       = [System.Drawing.Icon]::FromHandle((New-Object System.Drawing.Bitmap -Argument $stream).GetHIcon())
$Form.Width                      = $objImage.Width
$Form.Height                     = $objImage.Height
$Form.ForeColor                  = $formtextcolor

$buttomcount                     = 12 # Sets Form Height

# Hidden Panel
$Panel1                          = New-Object system.Windows.Forms.Panel
$Panel1.height                   = $buttomcount*37+10 # Button Count * Button Draw Size + Padding Bottom
$Panel1.width                    = 232
$Panel1.location                 = New-Object System.Drawing.Point(0,44)
$Form.Controls.Add($Panel1)

# Hidden Label
$Label1                          = New-Object system.Windows.Forms.Label
$Label1.text                     = "H I D D E N"
$Label1.AutoSize                 = $true
$Label1.width                    = 230
$Label1.height                   = 25
$Label1.location                 = New-Object System.Drawing.Point(50,13)
$Label1.Font                     = New-Object System.Drawing.Font('Berserker',16)
$Label1.ForeColor                = $textcolor
$Form.Controls.Add($Label1)

$position                        = 10 # Sets Each Button Location

# Rufus
$p1b1                            = New-Object system.Windows.Forms.Button
$p1b1.text                       = "Rufus"
$p1b1.width                      = 215
$p1b1.height                     = 35
$p1b1.location                   = New-Object System.Drawing.Point(10,$position)
$p1b1.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',12)
$p1b1.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b1)
$position += 37

# MSI Afterburner Config
$p1b2                            = New-Object system.Windows.Forms.Button
$p1b2.text                       = "MSI Afterburner Config"
$p1b2.width                      = 215
$p1b2.height                     = 35
$p1b2.location                   = New-Object System.Drawing.Point(10,$position)
$p1b2.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',12)
$p1b2.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b2)
$position += 37

# Discord Second Screen
$p1b3                            = New-Object system.Windows.Forms.Button
$p1b3.text                       = "Discord Second Screen"
$p1b3.width                      = 215
$p1b3.height                     = 35
$p1b3.location                   = New-Object System.Drawing.Point(10,$position)
$p1b3.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b3.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b3)
$position += 37

# Software RL
$p1b4                            = New-Object system.Windows.Forms.Button
$p1b4.text                       = "Software RL"
$p1b4.width                      = 215
$p1b4.height                     = 35
$p1b4.location                   = New-Object System.Drawing.Point(10,$position)
$p1b4.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b4.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b4)
$position += 37

# RGB Fusion
$p1b5                            = New-Object system.Windows.Forms.Button
$p1b5.text                       = "RGB Fusion"
$p1b5.width                      = 215
$p1b5.height                     = 35
$p1b5.location                   = New-Object System.Drawing.Point(10,$position)
$p1b5.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b5.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b5)
$position += 37

# JDK 17
$p1b6                            = New-Object system.Windows.Forms.Button
$p1b6.text                       = "JDK 17"
$p1b6.width                      = 215
$p1b6.height                     = 35
$p1b6.location                   = New-Object System.Drawing.Point(10,$position)
$p1b6.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b6.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b6)
$position += 37

# Eclipse IDE
$p1b7                            = New-Object system.Windows.Forms.Button
$p1b7.text                       = "Eclipse IDE"
$p1b7.width                      = 215
$p1b7.height                     = 35
$p1b7.location                   = New-Object System.Drawing.Point(10,$position)
$p1b7.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b7.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b7)
$position += 37

# Visual Studio Code
$p1b8                            = New-Object system.Windows.Forms.Button
$p1b8.text                       = "Visual Studio Code"
$p1b8.width                      = 215
$p1b8.height                     = 35
$p1b8.location                   = New-Object System.Drawing.Point(10,$position)
$p1b8.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b8.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b8)
$position += 37

# Dia
$p1b9                            = New-Object system.Windows.Forms.Button
$p1b9.text                       = "Dia"
$p1b9.width                      = 215
$p1b9.height                     = 35
$p1b9.location                   = New-Object System.Drawing.Point(10,$position)
$p1b9.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b9.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b9)
$position += 37

# Accent Color
$p1b10                            = New-Object system.Windows.Forms.Button
$p1b10.text                       = "Accent Color"
$p1b10.width                      = 215
$p1b10.height                     = 35
$p1b10.location                   = New-Object System.Drawing.Point(10,$position)
$p1b10.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b10.BackColor                  = $buttoncolor
$Panel1.Controls.Add($p1b10)
$position += 37

# EMPTY
$p1b11                            = New-Object system.Windows.Forms.Button
$p1b11.text                       = "EMPTY"
$p1b11.width                      = 215
$p1b11.height                     = 35
$p1b11.location                   = New-Object System.Drawing.Point(10,$position)
$p1b11.Font                       = New-Object System.Drawing.Font('Ubuntu Mono',11.5)
$p1b11.BackColor                  = $buttoncolor
#$Panel1.Controls.Add($p1b11)
$position += 37

# Go Back Button
$goback                          = New-Object system.Windows.Forms.Button
$goback.text                     = "Volver"
$goback.width                    = 215
$goback.height                   = 35
$goback.location                 = New-Object System.Drawing.Point(10,$position)
$goback.Font                     = New-Object System.Drawing.Font('Ubuntu Mono',12)
$goback.BackColor                = $bgcolor
$goback.ForeColor                = $textcolor
$Panel1.Controls.Add($goback)

$frompath                        = "https://github.com/Zarckash/FORMATEO/raw/main" # GitHub Downloads URL
$topath                          = "$env:userprofile\AppData\Local\Temp\FORMATEO" # Folder Structure Path
$download                        = New-Object net.webclient

# Rufus
$p1b1.Add_Click({
    $statusbox.text = "|Iniciando Rufus...`r`n" + $statusbox.text

    $p1b1.BackColor = $bgcolor
    $p1b1.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/PROGRAMAS/Rufus.exe", $topath+"\PROGRAMAS\Rufus.exe")
    Start-Process ($topath+"\PROGRAMAS\Rufus.exe")
    
    $p1b1.BackColor = $textcolor
    $p1b1.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Iniciando Rufus...        Hecho`r`n" + $statusbox.text
})

# MSI Afterburner Config
$p1b2.Add_Click({
    $statusbox.text = "|Configurando MSI Afterburner...`r`n" + $statusbox.text

    $p1b2.BackColor = $bgcolor
    $p1b2.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/CONF/Profiles.zip", $topath+"\CONF\Profiles.zip")
    Expand-Archive -Path ($topath+"\CONF\Profiles.zip") -DestinationPath 'C:\Program Files (x86)\MSI Afterburner\Profiles' -Force
    
    $p1b2.BackColor = $textcolor
    $p1b2.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Configurando MSI Afterburner...        Hecho`r`n" + $statusbox.text
})

# Discord Second Screen
$p1b3.Add_Click({
    $statusbox.text = "|Configurando Discord En El Monitor Derecho...`r`n" + $statusbox.text

    $p1b3.BackColor = $bgcolor
    $p1b3.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/CONF/settings.json", $topath+"\CONF\settings.json")
    Copy-Item -Path ($topath+"\CONF\settings.json") -DestinationPath $env:userprofile\AppData\Roaming\discord -Force
    
    $p1b3.BackColor = $textcolor
    $p1b3.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Configurando Discord En El Monitor Derecho...        Hecho`r`n" + $statusbox.text
})

# Software RL
$p1b4.Add_Click({
    $statusbox.text = "|Instalando Software RL...`r`n" + $statusbox.text

    $p1b4.BackColor = $bgcolor
    $p1b4.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/PROGRAMAS/RLSoftware.exe", $topath+"\PROGRAMAS\RLSoftware.exe")
    Start-Process ($topath+"\PROGRAMAS\RLSoftware.exe")
    
    $p1b4.BackColor = $textcolor
    $p1b4.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando Software RL...        Hecho`r`n" + $statusbox.text
})

# RGB Fusion
$p1b5.Add_Click({
    $statusbox.text = "|Instalando RGB Fusion...`r`n" + $statusbox.text

    $p1b5.BackColor = $bgcolor
    $p1b5.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/SCRIPTS/DOWNLOADS/RGBFusion.ps1", $topath+"\SCRIPTS\DOWNLOADS\RGBFusion.ps1")
    Start-Process powershell -ArgumentList "-noexit -windowstyle minimized -command powershell.exe -ExecutionPolicy Bypass $env:userprofile\AppData\Local\Temp\FORMATEO\SCRIPTS\DOWNLOADS\RGBFusion.ps1 ; exit"
    
    $p1b5.BackColor = $textcolor
    $p1b5.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando RGB Fusion...        Hecho`r`n" + $statusbox.text
})

# JDK 17
$p1b6.Add_Click({
    $statusbox.text = "|Instalando JDK 17...`r`n" + $statusbox.text

    $p1b6.BackColor = $bgcolor
    $p1b6.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/SCRIPTS/DOWNLOADS/JDK17.ps1", $topath+"\SCRIPTS\DOWNLOADS\JDK17.ps1")
    Start-Process powershell -ArgumentList "-noexit -windowstyle minimized -command powershell.exe -ExecutionPolicy Bypass $env:userprofile\AppData\Local\Temp\FORMATEO\SCRIPTS\DOWNLOADS\JDK17.ps1 ; exit"
    
    $p1b6.BackColor = $textcolor
    $p1b6.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando JDK 17...        Hecho`r`n" + $statusbox.text
})

# Eclipse IDE
$p1b7.Add_Click({
    $statusbox.text = "|Instalando Eclipse IDE...`r`n" + $statusbox.text

    $p1b7.BackColor = $bgcolor
    $p1b7.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/SCRIPTS/DOWNLOADS/Eclipse.ps1", $topath+"\SCRIPTS\DOWNLOADS\Eclipse.ps1")
    Start-Process powershell -ArgumentList "-noexit -windowstyle minimized -command powershell.exe -ExecutionPolicy Bypass $env:userprofile\AppData\Local\Temp\FORMATEO\SCRIPTS\DOWNLOADS\Eclipse.ps1 ; exit"
    
    $p1b7.BackColor = $textcolor
    $p1b7.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando Eclipse IDE...        Hecho`r`n" + $statusbox.text
})

# Visual Studio Code
$p1b8.Add_Click({
    $statusbox.text = "|Instalando Visual Studio Code...`r`n" + $statusbox.text

    $p1b8.BackColor = $bgcolor
    $p1b8.ForeColor = $textcolor

    winget install -h --force --accept-package-agreements --accept-source-agreements -e --id Microsoft.VisualStudioCode | Out-Null
    
    $p1b8.BackColor = $textcolor
    $p1b8.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando Visual Studio Code...        Hecho`r`n" + $statusbox.text
})

# Dia
$p1b9.Add_Click({
    $statusbox.text = "|Instalando Dia...`r`n" + $statusbox.text

    $p1b9.BackColor = $bgcolor
    $p1b9.ForeColor = $textcolor

    winget install -h --force --accept-package-agreements --accept-source-agreements -e --id gnome.Dia | Out-Null
    
    $p1b9.BackColor = $textcolor
    $p1b9.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Instalando Dia...        Hecho`r`n" + $statusbox.text
})

# Accent Color
$p1b10.Add_Click({
    $statusbox.text = "|Estableciendo Color De Enfasis #26FFDF...`r`n" + $statusbox.text

    $p1b10.BackColor = $bgcolor
    $p1b10.ForeColor = $textcolor

    $download.DownloadFile($frompath+"/PROGRAMAS/AccentColor.reg", $topath+"\PROGRAMAS\AccentColor.reg")
    regedit /s $env:userprofile\AppData\Local\Temp\FORMATEO\PROGRAMAS\AccentColor.reg
    
    $p1b10.BackColor = $textcolor
    $p1b10.ForeColor = $bgcolor

    $statusbox.text = "|Ready`r`n|Estableciendo Color De Enfasis #26FFDF...        Hecho`r`n" + $statusbox.text
})

# Go Back Button
$goback.Add_Click({
    $Form.Close()
})

[void]$Form.ShowDialog()